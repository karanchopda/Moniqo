  ...
  return (
    <div className="card card-hover flex flex-col items-start gap-6 h-full p-6 md:p-8">
      ...
      <div className="w-full mt-auto pt-6">
        <div className="w-full bg-gray-50 rounded border border-gray-100 p-4 space-y-3">
          {[
            ...
            <div key={label} className="flex items-center justify-between gap-3 rounded bg-white border border-gray-100 px-3 py-3">
              <div>
                <p className="text-sm font-bold text-primary">{label}</p>
                <p className="text-xs text-muted mt-1">{value}</p>
              </div>
              <div className={`w-9 h-5 rounded p-0.5 ${active ? 'bg-accent' : 'bg-gray-200'}`}>
                <div className={`w-4 h-4 rounded bg-white transition-transform ${active ? 'translate-x-4' : ''}`} />
              </div>
            </div>
          ))}
        </div>

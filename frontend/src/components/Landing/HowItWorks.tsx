      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 sm:gap-8 md:gap-12">
          {steps.map((step, i) => (
            ...
            >
              <div className="relative w-16 h-16 sm:w-20 sm:h-20 rounded-lg sm:rounded-xl bg-accent/10 flex items-center justify-center mb-4 sm:mb-6 transition-colors duration-200 group-hover:bg-accent/20">
                <span className="material-symbols-outlined text-accent text-3xl sm:text-4xl">
                  {step.icon}
                </span>
                <div className="absolute -top-2 -right-2 w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-primary flex items-center justify-center text-xs font-bold text-accent">
                  {i + 1}
                </div>
              </div>

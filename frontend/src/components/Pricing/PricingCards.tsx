...
export default function PricingCards() {
  ...
  return (
    <section className="px-8 pb-32 max-w-[1550px] mx-auto grid md:grid-cols-2 gap-12 items-stretch pt-32">
      {tiers.map((tier, i) => (
        <motion.div 
          key={i}
          ...
          className={`p-10 md:p-20 rounded-[4rem] flex flex-col justify-between border-2 transition-all duration-700 relative group h-full ${
            tier.highlight 
        ...
        >
          {tier.highlight && (
            <div className="absolute top-0 right-20 -translate-y-1/2 bg-accent px-8 py-3 rounded-full text-[10px] font-black uppercase tracking-[0.4em] text-primary shadow-2xl">
          ...
          <Link href="/signup" className="w-full">
            <button className={`w-full py-8 rounded-full font-headline font-black text-xs uppercase tracking-[0.4em] transition-all transform active:scale-[0.98] ${
              tier.highlight 

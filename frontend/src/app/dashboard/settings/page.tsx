  ...
  return (
    <div className="mx-auto w-full max-w-[1588px] px-4 py-7 sm:px-6 lg:px-11 lg:py-9">
      ...
      <div className="mb-7">
        ...
        <p className="mt-1 text-sm font-medium text-brand-text-muted">Manage your profile details, notifications, security, and subscription.</p>
      </div>
            ...
            return (
              <button
                ...
                className={`flex w-full items-center gap-3 rounded-md px-4 py-3 text-left text-sm font-bold transition-all ${
                  isActive
                    ? 'bg-brand-light text-primary shadow-sm'
                    : 'text-brand-text-muted hover:bg-brand-bg-light hover:text-brand-dark'
                }`}
              >
                <Icon className={`h-5 w-5 ${isActive ? 'text-brand-green-text-alt' : ''}`} />
                <span>{tab.label}</span>
              </button>
            );
          })}

          <div className="my-5 border-t border-brand-border-ultra-light"></div>

          {/* Quick Logout Button in settings menu */}
          <button
            onClick={logout}
            className="flex w-full items-center gap-3 rounded-md px-4 py-3 text-left text-sm font-bold text-brand-red transition-all hover:bg-rose-50"
          >
        ...
        <main className="space-y-6">
          ...
          {errorStatus && (
            <div className="flex items-center gap-2 rounded-md border border-brand-red-border bg-brand-bg-red p-4 text-sm font-bold text-brand-red shadow-sm animate-fadeIn">
              <Shield className="h-5 w-5 shrink-0 text-brand-red-bright" />
            ...
            <div className="rounded-md border border-brand-border-gray bg-white p-6 shadow-[0_8px_22px_rgba(15,23,42,0.03)] lg:p-8">
              ...
              <div className="mt-8 flex flex-col gap-6 sm:flex-row sm:items-center">
                ...
                <div>
                  ...
                  <p className="text-xs font-semibold text-brand-green-inactive">Personal Account</p>
              ...
              <form onSubmit={handleSaveProfile} className="mt-8 max-w-xl space-y-5">
                ...
                <button
                  type="submit"
                  className="mt-2 inline-flex h-11 items-center justify-center gap-2 rounded-md bg-primary px-6 text-sm font-black text-white shadow-sm hover:bg-primary-light transition-all"
                >
          ...
          {activeTab === 'notifications' && (
            <div className="rounded-md border border-brand-border-gray bg-white p-6 shadow-[0_8px_22px_rgba(15,23,42,0.03)] lg:p-8">
              ...
              <p className="mt-1 text-sm font-medium text-brand-text-muted">Control how and when you want to receive alerts.</p>
              ...
              <div className="mt-8 space-y-6 max-w-2xl">
                {[
                  ...
                  <div key={item.id} className="flex items-start justify-between gap-6 py-4 border-b border-brand-border-ultra-light last:border-0">
                    ...
                    <button
                      ...
                      className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${
                        item.state ? 'bg-brand-green-text-alt' : 'bg-brand-bg-gray-green'
                      }`}
                    >
                      <span
                ...
                <div className="pt-2">
                  <button
                    ...
                    className="inline-flex h-11 items-center justify-center gap-2 rounded-md bg-primary px-6 text-sm font-black text-white shadow-sm hover:bg-primary-light transition-all"
                  >
                    <Save className="h-4 w-4" />
                    Save Preferences
                  </button>
          ...
          {activeTab === 'security' && (
            <div className="rounded-md border border-brand-border-gray bg-white p-6 shadow-[0_8px_22px_rgba(15,23,42,0.03)] lg:p-8">
              ...
              <p className="mt-1 text-sm font-medium text-brand-text-muted">Manage password settings and privacy credentials.</p>

              <form onSubmit={handleSaveSecurity} className="mt-8 max-w-xl space-y-5">
                <div className="space-y-2">
                  <label htmlFor="currPass" className="text-sm font-bold text-brand-text-dark-blue">Current Password</label>
                  <input
                    id="currPass"
                    type="password"
                ...
                <div className="space-y-2">
                  <label htmlFor="newPass" className="text-sm font-bold text-brand-text-dark-blue">New Password</label>
                  <input
                    id="newPass"
                    type="password"
                    value={newPassword}
                ...
                <div className="space-y-2">
                  <label htmlFor="confPass" className="text-sm font-bold text-brand-text-dark-blue">Confirm New Password</label>
                  <input
                    id="confPass"
                    type="password"
          ...
          {activeTab === 'billing' && (
            <div className="rounded-md border border-brand-border-gray bg-white p-6 shadow-[0_8px_22px_rgba(15,23,42,0.03)] lg:p-8">
              <h2 className="text-lg font-black text-brand-dark">Billing & Subscription Plan</h2>
              <p className="mt-1 text-sm font-medium text-brand-text-muted">View details about your current subscription tier.</p>
                  ...
                  <div className="flex flex-wrap items-start justify-between gap-4">
                    <div>
                      <span className="inline-flex items-center gap-1 rounded bg-brand-green-bg-light px-2.5 py-1 text-xs font-black text-brand-green-medium">
                        <Sparkles className="h-3.5 w-3.5 fill-brand-green-medium/15" />
                        MONIQO PRO
                      </span>
                      <h3 className="mt-3 text-xl font-black text-brand-dark">₹499 / month</h3>
                    ...
                    <div className="rounded border border-brand-border-green-light bg-white px-5 py-4 text-center shadow-sm min-w-[150px]">
                      <p className="text-[10px] font-black text-brand-green-inactive uppercase tracking-wider">Next Payment</p>
                      <p className="mt-1 text-sm font-black text-brand-dark">03 July 2026</p>
                      <p className="mt-1 text-[11px] font-bold text-brand-green-text-alt">Auto-renews</p>
                    </div>
                  </div>
                </div>

                <div className="mt-6 flex flex-wrap gap-4">
                  <button className="inline-flex h-11 items-center justify-center rounded-md bg-primary px-6 text-sm font-black text-white shadow-sm hover:bg-primary-light transition-all">
                    Manage Payment Methods
                  </button>
                  <button className="inline-flex h-11 items-center justify-center rounded-md border border-brand-border bg-white px-6 text-sm font-black text-brand-text-muted hover:bg-brand-bg-light transition-all">
          ...
          <div className="rounded-md border border-rose-100 bg-rose-50/20 p-6 lg:p-8">
            ...
            <p className="mt-1 text-sm font-medium text-brand-text-muted">Safely disconnect from Moniqo and clear current session credentials.</p>
            <button
              onClick={logout}

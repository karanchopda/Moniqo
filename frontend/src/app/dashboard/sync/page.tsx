...
export default function SyncPage() {
  ...
  return (
    <div className="space-y-6 pb-4 font-sans">
      ...
      <div className="bg-[#3e5f4f] text-white p-6 rounded-2xl shadow-md flex flex-col md:flex-row justify-between items-start md:items-center gap-6 relative overflow-hidden border border-emerald-950/20">
        <div className="absolute right-0 bottom-0 w-32 h-32 rounded-tl-full bg-white/5 pointer-events-none select-none"></div>

        <div className="flex items-start gap-4">
          <div className="w-10 h-10 rounded-xl bg-white/10 flex items-center justify-center text-[#a3e8cc] shrink-0 mt-0.5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {connections.map((c) => (
          <div 
            key={c.id} 
            className={`bg-white border rounded-2xl p-6 shadow-sm flex flex-col justify-between space-y-6 transition-all ${
              c.highlighted 
          ...
          >
            ...
            <div className="flex justify-between items-start">
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-bold text-sm shadow-sm ${c.iconBg}`}>
                {c.iconText}
              </div>
              ...
              <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-tight ${
                ...
                <span className={`w-1 h-1 rounded-full ${c.status === 'Synced' ? 'bg-emerald-500' : 'bg-red-500'}`}></span>
                {c.status}
              </span>
            </div>
        ...
        <div className="bg-[#fcfdfd] border-2 border-dashed border-gray-200 rounded-2xl p-6 flex flex-col items-center justify-center text-center space-y-3 cursor-pointer hover:bg-gray-50/50 transition-colors min-h-[220px]">
          <div className="w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center text-gray-400 border border-gray-200/60 shadow-inner">
